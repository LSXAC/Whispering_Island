...
public partial class Player : CharacterBody2D
{
	...
	private float velo_x = 0, velo_y = 0;
	private string current_direction = "Down";
	private ShaderMaterial outline_shader = ResourceLoader.Load<ShaderMaterial>("res://Outline_Shader.tres");
	[Export]
	...
	private PackedScene action_info_box = ResourceLoader.Load<PackedScene>("res://Prefabs/action_info_box.tscn");
	...
	public override void _PhysicsProcess(double delta)
	{
		...
		if(Game_Manager.InsideGameMenu || Game_Manager.in_building_mode)
			return;

		if(Input.IsActionJustReleased("Zoom_In"))
			if((cam.Zoom + new Godot.Vector2(0.15f,0.15f)) <= new Godot.Vector2(max_zoom_offset,max_zoom_offset))
