...
public partial class BeltCombiner : Belt
{
    [Export]
    ...
    public void CheckTimerTimeout()
    {
        ...
        if (row == 0)
            ...
            {
                if (belt_0.item_holder.GetBeltItem().moving)
                    return;
                var item = belt_0.item_holder.offload_item();
                item_holder.GetParent<Belt>().receive_item(item);
                return;
            }
        ...
        if (row == 1)
            ...
            {
                if (belt_1.item_holder.GetBeltItem().moving)
                    return;
                var item = belt_1.item_holder.offload_item();
                item_holder.GetParent<Belt>().receive_item(item);
                return;
            }
        ...
        if (row == 2)
            ...
            {
                if (belt_2.item_holder.GetBeltItem().moving)
                    return;
                var item = belt_2.item_holder.offload_item();
                item_holder.GetParent<Belt>().receive_item(item);
                return;
            }
    }
